...
## Referencia al Resource original (EnemyData o CoreutaData)
var data: Resource
...
func _init(_data: Resource = null):
	if _data == null:
		return
	data = _data
	if _data is CoreutaData:
		_init_Coreuta(_data as CoreutaData)
...
func _init_Coreuta(kd: CoreutaData) -> void:
